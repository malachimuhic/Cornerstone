...
int main(void) {
    ...
    matrix* test_labels = mat_create(perm_arena, 60000, 10);    

    // draw_mnist_digit(train_images->data + 1 * 748);
    {
...
// TODO: Custom Alloc feature, Dynamically Adjust data allocation/output for arithmetic functions
b32 mat_add(matrix* out, const matrix* a, const matrix* b){
...
// n -> denotes transposed, t -> denotes NOT transpose(d)
...
b32 mat_mul(
    ...
    u32 a_cols = transpose_a ? a->cols : a->rows;
    u32 b_rows = transpose_b ? a->cols : a->rows;
    u32 b_cols = transpose_b ? a->cols : a->rows;
...
b32 mat_softmax(matrix* out, const matrix* in){
    // output @(index_i) = e^a_i / sum(e^a_i)
    if (out->rows != in->rows || out->cols != in->cols){
        return false;
    }
...
// loss function
// p == expected probability distribution WHILE q is actual
b32 mat_cross_entropy(matrix* out, const matrix* p, const matrix* q){
...
b32 mat_relu_grad(matrix* out, const matrix* in);
b32 mat_softmax_add_grad(
    matrix* out, const matrix* softmax_out, const matrix* grad
);
b32 mat_cross_entropy_add_grad(
